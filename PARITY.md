...
| **Перезапуск после обновления поднимает агента всегда** (ждёт смерти старого процесса, признак фона — аргументом) | ✅ (build 267) `/bin/sh`-помощник ждёт PID → `open -n [-g] <app> --args [--background] --relaunched-from <pid>`, `RIMEO_BACKGROUND`/`XPC_SERVICE_NAME` из окружения убраны, вывод в `relaunch.log`; гард в `main.swift` гасит новый экземпляр, если к этому моменту жив другой | ✅ bat ждёт 2 с → `start … --background` | До фикса macOS делал `open <app>` и сразу `exit(0)`: без `-n` LaunchServices не создавал второй экземпляр, пока старый жив, унаследованный `RIMEO_BACKGROUND=1` заставлял новый молча выйти на гарде `main.swift`, а процесс вне launchd-задачи с унаследованным `XPC_SERVICE_NAME` агента падает с SIGTRAP (проверено вживую). `-g` — не уводить фокус из Rekordbox при фоновом перезапуске. Итог — агент после обновления из iOS мог не подняться (эпизод 10.09). Тесты: `UpdateRelaunchTests` |
| Перезапуск после обновления возвращает окно, только если оно было на экране | ✅ (build 270, уточнено в 272) `UpdateChecker.relaunchInBackground` по `mainWindowShown` И `hasOnScreenWindow` (CGWindowList по своему pid); `open -g` всегда — перезапуск не отнимает фокус | ⚠️ по режиму ЗАПУСКА (`AgentSettings.LaunchedInBackground`), видимость окна не учитывается | Разница мелкая: на Windows агент, поднятый автозапуском, после обновления возвращается без окна, даже если окно было открыто; запущенный руками — с окном, даже если его закрыли.  До 270 признак фона сбрасывался навсегда при первом открытии окна: открыли и закрыли — после обновления окно всплывало и уводило фокус (замечено на 268→269). Действует с обновления 270→271: перезапуск выполняет код старой версии |
...
| **Быстрый старт хелпера синка: onedir, без frida** (build 270, задача #94) | ✅ `Contents/Resources/rbdb-sync-helper/` | ✅ `rbdb-sync-helper\` рядом с `RimeoAgent.exe` (+ старый exe запасным путём) | Был PyInstaller **onefile**: на КАЖДЫЙ Sync распаковывал ~143 МБ во временную папку (96 МБ — ненужная frida), система заново проверяла свежие .so/.pyd. Замер на маке: ответ Sync 38,9 с при ~1 с реальной записи; новый хелпер — 1,1 с на ту же работу. ⚠️ Windows проверен только сборкой логики на macOS и дымовым прогоном хелпера на CI — живой Sync на Windows после релиза проверить руками |
| Пустой `-wal` не сбивает кеш библиотеки | ✅ `RekordboxParser.dbMtime` | ✅ `RekordboxParser.DbMtime` | Наш же разбор создаёт пустой `master.db-wal` → mtime менялся → следующий `/api/data` перепарсивал только что разобранную базу (после Sync +10,6 с) |
...
| **Relay-опрос сам выходит из зависшего соединения** (без рестарта агента) | ✅ `URLSession.shared` сам переподключается при смене сети | ✅ (build 268) `SocketsHttpHandler` с `PooledConnectionLifetime` 2 мин + пересоздание клиента после 3 ошибок подряд | До 268 Windows держал один `HttpClient` с бессрочным пулом: после сна/смены сети опрос часами падал в «A task was canceled» (17 ч у агента 260), лечилось только рестартом |
| **403 от WAF/прокси на relay-опросе не разлогинивает** (к выходу считается только JSON-ответ облака с `reason`) | ✅ (build 270) `CloudRelay.appReason403` + тесты | ✅ (build 270) `CloudRelay.AppReason403` | До 270 три 403 подряд без `reason` (HTML-страница WAF) разлогинивали агента: одно новое WAF-правило на rimeo.app выкинуло бы из аккаунта весь парк (#96) |
| **Осиротевший коннектор туннеля уходит ДО запуска своего** (нет двух коннекторов на одном туннеле) | ✅ (build 269) SIGTERM → ждём до 3 с → SIGKILL (`TunnelManager.terminateAndWait`, тесты `TunnelReapTests`) | ✅ `Kill(true)` сразу | До фикса macOS слал SIGTERM и сразу запускал свой коннектор: пока сирота закрывала соединения, Cloudflare раскидывал запросы на два коннектора — заикание стрима после рестарта/обновления агента (#6) |
...
| **XML-источник отдаёт папки плейлистов** (`is_folder`/`parent`/`seq`, пустые папки тоже; порядок = документ экспорта) | ✅ (build 271) `RekordboxParser.parseXML` + `XMLPlaylistFoldersTests` | ✅ (build 271) `RekordboxParser.ParseXml` | До 271 XML-ветка только обходила папки: наружу шли одни плейлисты без полей дерева (mac — в случайном порядке, Windows — по алфавиту). Мобильный веб у XML-пользователей не видел ничего внутри папок, у iOS не было списка папок. `rekordbox_id` у XML-записей нет (XML только на чтение). Веб-часть — v1.34.5 (#17) |
| **LAN-адрес агента по heartbeat релея** (`&lan=ip:port`) → облако обновляет подсказку `lan_ip` | ✅ (build 269) `CloudRelay.lanHint` | ✅ (build 269) `CloudRelay.LanHint` (+ только если сервер слушает все интерфейсы, `AgentHttpServer.LanEnabled`) | Облако принимает с v1.34.3 (`_lan_hint_url`, только RFC1918, пишет при изменении). Раньше адрес писался только при логине и протухал после смены IP — для Windows это единственный путь телефона по локалке (mDNS нет). Схему `http://` не шлём: триггер WAF, а его 403 агент считает отказом токена |
| **Причина 403 path-guard пишется в лог** (`/stream`, `/waveform`, `/artwork`, `/reveal`) | ✅ | ✅ (build 269) | До 269 Windows отвечал голым 403 без записи — «пачки 403 на /artwork» (#8) было не разобрать |
| **Кеш разрешённых корней path-guard пересобирается при каждом переразборе библиотеки** | ✅ (build 272) `LibraryPathGuard.rootsSignature` (источник + mtime + число/первый/последний), корни из того же снимка + `PathGuardRootsTests` | ✅ (build 272) `LibraryPathGuard.RootsSignature` | До 272 ключ был «число | первый | последний путь»: трек переехал (Relocate), а эти значения не изменились — агент отдавал новый путь и сам отвечал на него 403 (#8). Пересчёт ~4 мс на 2410 треков |
...
|---|---|---|---|
| **C1** pairing_info не отдаёт LAN PSK по сети | ✅ гейт `req.trusted` (только in-process UI) + пин-тест «loopback-пир с CF-заголовками → 403» | ✅ (build 267) ручка **закрыта для сети целиком** (403 всем, убрана из `PublicPaths`) | ⚠️ Прежний Windows-гейт `PeerIsLoopback` был дырой: cloudflared и CloudRelay ходят в агента с 127.0.0.1, поэтому через публичный туннель ручка отдавала PSK **и свежий облачный `mobile_token`**. Ручку не зовёт никто (QR-паринг у агента убран), код сборки QR удалён. На macOS оставлена как была (in-process only, тоже никем не вызывается) |
...
| **H1** QR-секрет не уходит в api.qrserver.com | ✅ нейтрализован C1 (UI генерит QR локально) | ✅ (2026-09-21) код pairing-QR с PSK удалён вместе с pairing_info | В api.qrserver.com у Windows-UI уходит только ссылка на скачивание приложения (`AccountPage`), секретов в ней нет |
| **PSK действует только из LAN и с этой машины** (не через туннель / CloudRelay / публичный адрес) | ✅ (build 267) `AccessControl.acceptsPSK(transport:)` + 8 тестов | ✅ (build 267) `AccessControl.AcceptsPsk` | PSK не ротируется; раньше утёкший ключ = вечный удалённый доступ через туннель. Канал — `Transport.classify` по адресу пира из ядра (туннель/relay распознаются только у loopback-пира). WinUI ходит с PSK через 127.0.0.1 → канал LOCAL → работает. Отказ пишется в лог агента как `auth=deny:psk_not_accepted_via_<канал>`; клиенту — та же причина, что и на неверный ключ (иначе 401 через туннель подтверждал бы, что утёкший ключ настоящий). Задача LAN-auth v2, шаг 1 (`memory/tasks/lan-auth-v2.md`) |
| **Адрес туннеля не отдаётся анонимно** (`/api/status`, `/api/account`, `/api/tunnel/status`) | ✅ (build 267) | ✅ (build 267) | Анонимный `tunnel_url` давал любому в Wi-Fi постоянный публичный адрес агента. Потребителей анонимного поля нет (iOS/облако/веб его не читают; проверки готовности туннеля смотрят только код ответа) |
